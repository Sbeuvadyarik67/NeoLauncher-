...
class NeoBrainLauncher:
    ...
    def load_manifest(self):
        """Загружает manifest.json с отладкой"""
        try:
            if os.path.exists(self.manifest_path):
                with open(self.manifest_path, 'r', encoding='utf-8') as f:
                    ...
                    for pid, pdata in data.get('projects', {}).items():
                        print(f"   📁 {pid}: {pdata.get('path', 'Нет пути')}")
                    return data
            else:
                print("❌ Файл manifest.json не найден!")
                # Создаём дефолтный манифест
                default = {
                    "projects": {
                        "neobrain": {
                            "name": "NeoBrain",
    ...
    def setup_ui(self):
        ...
        footer_text = tk.Label(
            footer,
            text="✦ ВСЕ ПРОЕКТЫ В ОДНОЙ ПАПКЕ — ОБНОВЛЯЙ ПЕРЕТАСКИВАНИЕМ ✦",
            font=("Segoe UI", 9),
            bg=self.bg_dark,
            fg="#2a2a5a"
        )
    ...
    def get_project_status(self, project_id, project_data):
        ...
        if os.path.exists(project_path):
            return {"text": "✦ ГОТОВ К ЗАПУСКУ", "color": "#10b981"}
        else:
            return {"text": "✧ ФАЙЛ НЕ НАЙДЕН", "color": "#ff2d8a"}
